      ...
      <div>
         <v-card class="mt-3">
             <v-card-title v-if="tableTitle !== ''">
              ...
              item-key="MAN_MAN_GEN_CSA_ID"             
              expand           
            >
              <template slot="items" slot-scope="props">
                 <tr   @click="dobleClick($event,props.item,props.index,props)" >
                  <td class="justify-center layout px-0" v-if="noEdit !== false">
                  <v-icon
                      small
                      class="mr-2"
               ...
               <template slot="pageText" slot-scope="props">
                  Página {{ props.pageStart }} - {{ props.pageStop }} de {{ props.itemsLength }}
                    ...
                    <template slot="expand" >
                      <v-card>
                        <v-card-text>
                          <tableAutomaticSelected 
                            v-bind="{
                            service:subService,
                            idName:subIdName,
                            constraint:subConstraint,
                            tableTitle:subTableTitle,
                            columns:subColumns,
                            noColumns:subNoColumns,
                            noAdd:subNoAdd,
                            noEdit:subNoEdit,
                            withChekbox:subWithChekbox,
                            expand:subExpand}" />
...
//            mdbCard,mdbCardBody,mdbCardTitle,mdbCardText,mdbCardUp,mdbView,ViewWrapper} from 'mdbvue';
  ...
  import tableAutomaticSelected from '@/components/tableAutomaticSelected.vue'
...
export default {
    ...
    updated(data) {
      ...
      insetData[this.idName] = this.itemsUnchanged[this.indexTableUbdateSelected][this.idName];
      console.log({insetData});     
    ...
    deleted(){
      ...
      for (let i = 0; i < this.fields.length; i++) {
        if(this.fields[i].text.toLowerCase() !== 'editar'){
            if(iAux){
    ...
    let id = this.itemsUnchanged[this.indexTableUbdateSelected][this.idName]
    console.log({id});     
    ...
    selectCorrectConstraint:async function(){
        for (let i = 0; i < this.constraint.length; i++) {    
            if(this.constraint[i].service){            
              ...
              this.fields[index].text = this.constraint[i].label;             
    ...
    selectCorrectColumns: async function(){
        ...
        if(this.columns.length > 0){
         for (let i = 0; i < this.fields.length; i++) { 
               ...
               let indexCol = this.columns.findIndex(col =>col.key === this.fields[i].value);
                if(indexCol === -1){                    
                    ...
                    this.fields[i].text = this.columns[indexCol].label;
    ...
    //   datePicker,
      ...
      // variables de control de doble clic -->
        ...
        // <--variables de control de doble clic
                ...
                subTableTitle:"",          
                subService:'manMantenDetalle',
                subIdName:'MAN_MDT_ID',
                subColumns:[
                    // {
                    //     key:'GEN_COM_CODIGO',
                    //     label:'Codigo'
                ...
                // noColumns:['GEN_EST_ID_TABLA',],// son las columas que no se quieren mostrar
                subNoColumns:[],
                subConstraint:[
                //     {
                ...
                //   },
                //   {
                //     localReference:'GEN_EST_REG_ACTIVO',
                //     reference:null,
